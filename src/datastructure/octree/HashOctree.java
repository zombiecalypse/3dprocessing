...
/**
 * This is an implementation of a hashtable based Octree. The Morton Codes used
 * are described in detail in the slides accompanying this exercise, the bitwise
 * operations on them will be implemented in the class {@link MortonCodes}.
 * <p>
 * To navigate through the adjacencies of the octree, the methods getNbr_*2* can
 * be used (once implemented). Their arguments Oxyz encode relative difference
 * vectors, which usually will be 3-bit integers. For example the argument 0b011
 * in the method getNbr_c2c(cell, 0b011) would encode that the cell at the
 * relative grid position +0x, +1y , +1z is sought (relative to the cell passed
 * as parameter).
 * 
 * @author Alf
 * 
...
public class HashOctree {
	...
	/**
	 * Abort criterion for the hashoctree construction: stop refining at depth
	 * maxDepth MaxDepth should never be more than 20, as longs have 64 bit, and
	 * every level needs 3 bits and 4 additional bits are needed to discover
	 * overflows.
	 */
	private int maxDepth;
	/**
	 * Abort criterion for the hashoctree refinement: stop refining if a cell
	 * has less than pointsPerCell points.
	 */
	...
	/**
	 * For convenience, all leafs of the octree (cells with no sub-cells) are
	 * stored here. This allows to enumerate them and access them by an index
	 * (useful for matrix construcion)
	 */
	private ArrayList<HashOctreeCell> leafs;
	/**
	 * For convenience, all vertices of the octree are stored here. This allows
	 * to enumerate them and access them by an index (useful for matrix
	 * construcion)
	 */
	 ...
	 * 
	 * <p>
	 * Construct a hashbased octree for the PointCloud pc. depth denotes the
	 * maximal depth the octree can have, and pointsPerCell controls at what
	 * number of points a cell is not split any further.
	 * </p>
	 * <p>
	 * The factor parameter controls how many times as big as the boundingbox of
	 * the pointcloud the hashtree will be; if factor is < 1 the argument is
	 * ignored and 1 is used instead.
	 * </p>
	 * 
	 * @param pc
	 * @param depth
	...
	public HashOctree(PointCloud pc, int depth, int pointsPerCell, float factor) {
		if (depth > MAX_DEPTH)
			throw new IllegalArgumentException(String.format(
					"Depth should not be bigger than %i but was %i", MAX_DEPTH,
					depth));
		...
		bboxMax = root.computeVertexPos(0b111);
		bboxMin = root.computeVertexPos(0b000);
		...
		// this creates a hashmap of all octree vertices. This method makes sure
		// that every
		// vertex is registered exactly once, even if it is used on multiple
		// levels by multiple cells
		buildVertexMap();

		// lastly: enumerate the vertices and leafs.
		// the vertices and leafs are stored a second time in two separate
		// arrays.
		// Leafs and Vertices have an 'index' field, this method assign to every
		// vertex and every leaf its unique integer index.
		// This index will be used during the Reconstruction assignment to build
		// and debug linear systems.
		enumerateVertices();
		enumerateLeafs();
	...
	/**
	 * First step int the hashoctree creation. This initializes the cells, the
	 * hashmap containing the cells and the list containing all Leafs The method
	 * operates in a recursive fashion on the stack.
	 * 
	 * @param stack
	 */
	private void buildTree(Stack<HashOctreeCell> stack) {
		...
		while (!stack.isEmpty()) {
			...
			if (!this.cellMap.containsKey(node.code)) {
				...
				assert (false); // sanity check no two cells should have the
								// same morton code!!!!
			}
	...
	private void buildVertexMap() {
		...
		long vertKey;
		// every vertex is the corner of some leaf. This method makes sure that
		// every vertex is registered once only.
		HashOctreeVertex v;

		for (HashOctreeCell c : leafs) {
			for (int i = 0b000; i <= 0b111; i++) {
				// compute the hash corresponding to the vertex 0bxyz: this is
				// the nbr code + padding
				vertKey = nbrCode(c.code, c.lvl, i) << (3 * (depth - c.lvl)); // getVertexHash(c,
																				// i);
				if (vertKey >= 0) {
	...
	/**
	 * This helper method creates the child octree cells, computes their morton
	 * codes and splits the points contained in node between the children,
	 * 
	 * @param node
	 * @param stack
	 */
	...
	private void enumerateVertices() {
		int i = 0;
		this.vertices = new ArrayList<HashOctreeVertex>(this.vertexMap.values()
				.size());
	...
	/**
	 * Retrieve the cell which corresponds to the morton code in the argument.
	 * This will return null if no such cell exists.
	 * 
	 * @param hash
	 * @return
	 */
	...
	/**
	 * Retrieve the vertex which corresponds to the morton code in the argument.
	 * This will return null if no such cell exists.
	 * 
	 * @param hash
	 * @return
	...
	public HashOctreeCell getNbr_v2c(HashOctreeVertex v, int nbr_Obxyz) {
		...
		code = nbrCodeMinus(code, v.maxLvl, 0b111 & (~nbr_Obxyz));
	...
	public HashOctreeCell getParent(HashOctreeCell cell) {
		return this.getCell(parentCode(cell.code));
	}

	/**
	 * Return an adjacent cell on the same or on a lower level, which lies in
	 * the direction Obxyz. If there is no such cell, null is returned
	 * 
	 * @param cell
	 * @param Obxyz
	 * @return
	...
	/**
	 * find and return a vertex on the finest grid possible, that shares an edge
	 * of some octreecell with the vertex v and lies in direction nbr_0bxyz
	 * (0b100 = +x direction, 0b010 = +y direction 0b001 = +z direction). If no
	 * neighbor exists, null is returned.
	 * 
	 * @param v
	 * @param nbr_0bxyz
	 * @return
	...
	/**
	 * find and return maximal depth vertex, that shares an edge of some
	 * octreecell with vertex and lies in direction nbr_0bxyz (0b100 = +x
	 * direction, 0b010 = +y direction 0b001 = +z direction). If no neighbor
	...
	public boolean isOnBoundary(HashOctreeVertex v) {
		return isVertexOnBoundary(v.code, this.depth);
	}

	/**
	 * Find and return the Leaf cell in which p lies. This method will return
	 * null if the point is outside of the volume covered by this octree.
	 * 
	 * @param p
	 * @return
	 */
	public HashOctreeCell getCell(Point3f p) {
		...
		if (p.x > bboxMax.x + 10e-5 || p.y > bboxMax.y + 10e-5
				|| p.z > bboxMax.z + 10e-5 || p.x < bboxMin.x - 10e-5
				|| p.y < bboxMin.y - 10e-5 || p.z < bboxMin.z - 10e-5) {
			return null;
		}
		long code = root.code;
